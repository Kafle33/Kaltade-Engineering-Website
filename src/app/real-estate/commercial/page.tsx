...
export default function CommercialRealEstatePage() {
  ...
  const commercialAssetTypes = [
    ...
    {
      title: 'Development Sites & Land Banks',
      icon: Layers,
      desc: 'Multi-acre acreage suitable for master-planned commercial subdivision, logistics yards, or institutional headquarters.',
      tag: 'Large Acreage',
    },
    {
      title: 'Investment Properties',
      icon: TrendingUp,
      desc: 'High-yield commercial assets offering capitalization rate security, corporate tenant leases, and long-term land appreciation.',
      tag: 'Yield Assets',
    },
  ];

  // How Kaltade adds value to commercial transactions through engineering diligence
  const engineeringValuePillars = [
    {
      title: 'Certified Commercial Valuation',
      icon: Scale,
      desc: 'We appraise commercial buildings and land using discounted cash flow (DCF), capitalized rental yield, and depreciated replacement cost models accepted by commercial banks.',
      connectedService: '/valuation',
      serviceName: 'Property Valuation',
    },
    {
      title: 'Feasibility & Highest-and-Best-Use (HBU)',
      icon: TrendingUp,
      desc: 'We calculate optimal floor zoning, tenant mix absorption rates, break-even timelines, and construction CapEx feasibility before you invest.',
      connectedService: '/real-estate/investment',
      serviceName: 'Investment Feasibility',
    },
    {
      title: 'Subdivision & Development Planning',
      icon: Layers,
      desc: 'For commercial acreage: internal road geometry, stormwater drainage, high-tension power connectivity, and plot yield maximization.',
    ...
    },
    {
      ...
      desc: 'Comprehensive project reports complete with architectural master plans, Bill of Quantities (BOQ), and multi-year financial modeling for loan syndication.',
      connectedService: '/services/dpr',
      serviceName: 'DPR & Project Reports',
    },
    {
      title: 'Architectural & NBC Structural Design',
      icon: Compass,
      desc: 'Structural engineering (ETABS seismic modeling conforming to NBC 105:2020), fire safety layouts, and basement parking circulation.',
      connectedService: '/services/engineering',
      serviceName: 'Engineering & Design',
    },
    {
      title: 'Pre-Purchase Technical Due Diligence',
      icon: ShieldCheck,
      desc: 'Physical boundary surveying, cadastral trace alignment, road right-of-way setback audits, and municipal compliance screening.',
      connectedService: '/real-estate/due-diligence',
      serviceName: 'Due Diligence Audit',
    },
  ];
  ...
  const handleInquireProperty = (prop: Property) => {
    ...
    setFormData((prev) => ({
      ...prev,
      commercialCategory: prop.type,
      preferredLocation: `${prop.location.address}, ${prop.location.city}`,
    }));
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newLead = saveLead({
      ...
      message: `[Transaction: ${formData.transactionType}] ${formData.message}${
        selectedProperty ? ` (Ref Property: ${selectedProperty.title} - ${selectedProperty.id})` : ''
      }`,
      urgency: 'Standard',
    });
  ...
  return (
    <div className="pt-28 sm:pt-32 bg-white dark:bg-dark-bg min-h-screen text-navy-950 dark:text-dark-text transition-colors">
          ...
          <div className="max-w-3xl">
            ...
            <div className="mt-10 pt-8 border-t border-white/10 dark:border-dark-border grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-300 dark:text-slate-400">Verified Road Right-of-Way</span>
              </div>
              <div className="flex items-center gap-2">
                <Scale className="w-4 h-4 text-amber-400 shrink-0" />
                <span className="text-slate-300 dark:text-slate-400">Certified Asset Valuation</span>
              </div>
              <div className="flex items-center gap-2">
                <FileSpreadsheet className="w-4 h-4 text-blue-400 dark:text-sky-400 shrink-0" />
                <span className="text-slate-300 dark:text-slate-400">Bankable DPR &amp; Feasibility</span>
              </div>
              <div className="flex items-center gap-2">
                <HardHat className="w-4 h-4 text-emerald-400 shrink-0" />
                <span className="text-slate-300 dark:text-slate-400">Structural NBC Compliance</span>
              </div>
            </div>
          </div>
        </div>
                ...
                >
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <div className="p-3 rounded-xl bg-navy-50 dark:bg-dark-elevated text-navy-900 dark:text-sky-300 group-hover:bg-navy-900 dark:group-hover:bg-navy-700 group-hover:text-white transition-colors">
                    ...
                    <h3 className="text-lg font-bold text-navy-950 dark:text-white mb-2 leading-snug">
                      {asset.title}
                    </h3>
                  ...
                  <div className="mt-6 pt-3 border-t border-slate-100 dark:border-dark-border">
                    <button
                      type="button"
                      onClick={() => {
                        setFormData((prev) => ({
                      ...
                      className="text-xs font-bold text-navy-900 dark:text-sky-400 hover:text-navy-700 dark:hover:text-sky-300 flex items-center gap-1 group/link cursor-pointer"
                    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
            {engineeringValuePillars.map((pil) => {
              ...
              return (
                ...
                >
                  <div>
                    <div className="p-3.5 w-fit rounded-2xl bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border text-navy-900 dark:text-white shadow-xs mb-6">
                  ...
                  <div className="pt-4 border-t border-slate-200 dark:border-dark-border">
                    <Link
                      ...
                      className="text-xs font-bold text-navy-900 dark:text-sky-400 hover:text-navy-700 dark:hover:text-sky-300 flex items-center justify-between group/svc"
                    >
                      <span>Explore {pil.serviceName}</span>
                      <ArrowRight className="w-4 h-4 group-hover/svc:translate-x-1 transition-transform text-amber-600 dark:text-amber-400" />
                    </Link>
                  </div>
                </div>
              );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-6 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2 text-xs">
                <div className="p-3 bg-white/5 dark:bg-dark-elevated/60 border border-white/10 dark:border-dark-border rounded-xl">
            ...
            <div className="lg:col-span-6 bg-white dark:bg-dark-card text-navy-950 dark:text-white p-8 sm:p-10 rounded-3xl shadow-2xl dark:shadow-card-dark border border-slate-200 dark:border-dark-border space-y-6">
              ...
              <div className="space-y-3 text-xs sm:text-sm">
                <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-dark-surface rounded-xl border border-slate-200 dark:border-dark-border">
      ...
      <section className="py-20 sm:py-28 bg-slate-100 dark:bg-dark-bg border-t border-slate-200 dark:border-dark-border text-center">
          ...
          <div className="py-8 text-center space-y-4">
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-dark-border hover:bg-slate-50 dark:hover:bg-dark-elevated text-xs font-semibold text-slate-700 dark:text-slate-300 cursor-pointer"
              >
                Close Window
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-slate-300 dark:border-dark-border rounded-lg focus:ring-2 focus:ring-navy-900 dark:focus:ring-sky-400 focus:outline-none bg-white dark:bg-dark-card text-slate-900 dark:text-white cursor-pointer"
                >
