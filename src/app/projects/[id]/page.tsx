  ...
  return (
    <div className="min-h-screen bg-slate-50/60 pt-28 sm:pt-32 pb-24">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        {/* Header Section */}
        <div className="bg-white rounded-3xl p-6 sm:p-10 lg:p-12 border border-slate-200 shadow-sm space-y-6">
          {/* Metadata Badges */}
          <div className="flex flex-wrap items-center gap-2.5">
            <span className="px-3 py-1 rounded-md bg-navy-950 text-white text-xs font-bold uppercase tracking-wider">
              {project.category}
            </span>
            <span className="px-3 py-1 rounded-md bg-navy-50 text-navy-900 text-xs font-semibold">
              Client: {project.clientType}
            </span>
            <span className="px-3 py-1 rounded-md bg-slate-100 text-slate-700 text-xs font-semibold flex items-center gap-1">
              <Calendar className="w-3.5 h-3.5 text-slate-400" />
              {project.year}
            </span>
            <span className="px-3 py-1 rounded-md bg-amber-50 text-amber-900 text-xs font-bold border border-amber-200/60 ml-auto font-mono">
              {project.id}
            </span>
          </div>
          ...
          {/* Location & Summary */}
          <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-500 font-medium pb-4 border-b border-slate-100">
        ...
        <div className="space-y-4">
          <div className="relative aspect-[16/9] w-full rounded-3xl overflow-hidden bg-slate-900 shadow-lg border border-slate-200">
            <Image
              src={
                project.images[0] ||
          ...
          {project.images.length > 1 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {project.images.slice(1).map((img, idx) => (
                <div
                  key={idx}
                  className="relative aspect-[16/10] rounded-2xl overflow-hidden border border-slate-200 shadow-sm bg-slate-100"
                >
                  <Image
                    src={img}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-4">
              <div className="flex items-center gap-2.5 text-navy-950 font-extrabold text-xl">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-emerald-200 bg-emerald-50/30 shadow-sm space-y-4">
              <div className="flex items-center gap-2.5 text-emerald-950 font-extrabold text-xl">
                <div className="w-8 h-8 rounded-lg bg-emerald-100 text-emerald-700 flex items-center justify-center shrink-0">
                  <CheckCircle2 className="w-5 h-5" />
                </div>
                <h2>The Outcome & Impact</h2>
              </div>
              <p className="text-emerald-950/90 leading-relaxed text-sm sm:text-base font-medium">
                {project.outcome}
              </p>
            </div>
          </div>
                ...
                <div className="divide-y divide-slate-100 text-xs">
                  {project.specifications.map((spec, sIdx) => (
                    <div key={sIdx} className="py-2.5 flex flex-col space-y-1">
                      ...
                      <strong className="text-navy-950 font-bold text-xs sm:text-sm">
                        {spec.value}
                      </strong>
                    </div>
                  ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {relatedProjects.map((rel) => (
                <Link
                  ...
                  className="group p-6 rounded-3xl bg-white border border-slate-200 shadow-sm hover:shadow-lg transition-all flex flex-col justify-between"
                >
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-md bg-navy-50 text-navy-900 text-[11px] font-bold uppercase">
                        {rel.category}
                      </span>
                    ...
                    <h4 className="text-base font-bold text-navy-950 group-hover:text-amber-600 transition-colors">
                      {rel.title}
                    </h4>
                  ...
                  <div className="pt-4 mt-2 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-navy-900 group-hover:text-amber-600">
